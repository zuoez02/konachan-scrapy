...
import urllib
from konachan.items import YandereItem
import logging
import json
import os
...
class YandereSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        cwd = os.getcwd()
        self.cacheFilePath = os.path.join(cwd, 'yandere_cache', self.folder + '.json')
    ...
    def parse(self, response):
        ...
        for post in posts:
            if post is not None:
                ...
                self.log("Post " + id + " not scrapied: " + response.urljoin(post), logging.INFO)
                yield response.follow(response.urljoin(post), callback=self.parseYandereDetail)
    ...
    def parseYandereDetail(self, response):
        url = response.request.url
        s = response.request.url.split('/')
        id = s[len(s) - 1]
        post = YandereItem()
        post['id'] = id
        links = response.css('li.tag-type-general a::text').getall()
        tag = [];
        i = len(links)
        for link in links:
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(YandereSpider, cls).from_crawler(crawler, *args, **kwargs)
